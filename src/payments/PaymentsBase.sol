...
abstract contract PaymentsBase is ReentrancyGuardUpgradeable, FacetInitializable, Modifiers, IPayments {
     ...
     */
    function __PaymentsBase_init(address _gasTokenUSDPriceFeed, address _trvAddress) internal onlyFacetInitializing {
        LibPayments.setGasTokenUSDPriceFeed(_gasTokenUSDPriceFeed);
        LibPayments.setTrvAddress(_trvAddress);
    }
     ...
     * @inheritdoc IPayments
     */
    function getTrvAddress() external view override returns (address trvAddress_) {
        trvAddress_ = LibPayments.getTrvAddress();
    }
    ...
    function _getPriceFeed(
        ...
        internal
        view
        returns (AggregatorV3Interface priceFeed_)
    {
    ...
    /**
     * @dev returns the given _price in the given decimal format after converting the _price into the related value from
     * the _price feed.
        ...
        returns (uint256 paymentAmount_)
    {
        ...
        // NOTE: It is assumed that the  _paymentAmountInPricedToken and the _price feed's _price are in the same
        // decimal unit
    ...
    function _getQuotePrice(AggregatorV3Interface _priceFeed) internal view returns (uint256 price_) {
        ...
        // Unfortunately no way to determine this ahead of time, and likely will never occur, but is a possibility of
        // the oracle
        if (_quotePrice < 0) {
