...
library LibGuildManager {
    ...
    function setTrevorTagNFTAddress(address _trevorTagNFTAddress) internal {
        GuildManagerStorage.Layout storage _l = GuildManagerStorage.layout();

        _l.trevorTagNFTAddress = _trevorTagNFTAddress;
    }
    ...
    function getUserInfo(
        ...
        internal
        view
        returns (GuildOrganizationUserInfo storage info_)
    {
    ...
    function setRequireTrevorTagForGuilds(bytes32 _organizationId, bool _requireTrevorTagForGuilds) internal {
        getGuildOrganizationInfo(_organizationId).requireTrevorTagForGuilds = _requireTrevorTagForGuilds;

        emit GuildManagerStorage.RequireTrevorTagForGuildsUpdated(_organizationId, _requireTrevorTagForGuilds);
    }
    ...
    function createGuild(bytes32 _organizationId) internal {
        if (getGuildOrganizationInfo(_organizationId).requireTrevorTagForGuilds) {
            requireTrevorTagHolder(LibMeta._msgSender());
        }
    ...
    function requireTrevorTagHolder(address _user) internal view {
        GuildManagerStorage.Layout storage _l = GuildManagerStorage.layout();

        if (IERC721Upgradeable(_l.trevorTagNFTAddress).balanceOf(_user) == 0) {
            revert GuildManagerStorage.UserDoesNotOwnTrevorTag(_user);
        }
    }
