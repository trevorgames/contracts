...
/**
 * @dev Used to track ERC20 payment feeds and decimals for conversions. Note that `decimals` equaling 0 means that the
 * erc20
 ...
 *      Ex. LINK / USD would give an answer of "How much USD equals 1 LINK?" -> 7.26 USD (See
 * https://coincodex.com/convert/chainlink/usd/)
 * @param pricedInGasTokenAggregator The price feed for ERC20 priced in gas tokens
 *      Ex. LINK / ETH would give an answer of "How much ETH equals 1 LINK?" -> 0.004 ETH (See
 * https://coincodex.com/convert/chainlink/ethereum/)
 * @param gasTokenPricedInERC20Aggregator The price feed for the gas token priced in the ERC20.
 *      There are currently no price feeds for this, however if one were to exist, it would be supportable
 *      Ex. ETH / LINK would give an answer of "How much LINK equals 1 ETH" -> 249.13 LINK (See
 * https://coincodex.com/convert/ethereum/chainlink/)
...
interface IPayments {
     ...
     * @param _paymentERC20 The address of the ERC20 to take
     * @param _paymentAmountInPricedToken The desired payment amount, priced in another token, depending on what
     * `priceType` is
     * @param _priceType The type of currency that the payment amount is priced in
     * @param _pricedERC20 The address of the ERC20 that the payment amount is priced in. Only used if `_priceType` is
     * PRICED_IN_ERC20
     */
     ...
     * @param _paymentToken The token to convert from. If address(0), then the input is in gas tokens
     * @param _paymentAmountInPricedToken The desired payment amount, priced in either the `_pricedERC20`, gas token, or
     * USD depending on `_priceType`
     *      used to calculate the output amount
     * @param _priceType The type of conversion to perform
     * @param _pricedERC20 The token to convert to. If address(0), then the output is in gas tokens or USD, depending on
     * `_priceType`
     */
    ...
    /**
     * @return trvAddress_ The address of the $TRV contract
     */
    function getTrvAddress() external view returns (address trvAddress_);
}
